...
class POT():
    ...
    def calcThreshold(self,data):
        ...
        threshold_list = []
        threshold_list.append(data[RequiredHeaders.WAVE_SIG_HEIGHT].quantile(self.STORM_THRESHOLD_PARAMETER))
    ...
    def peakOverThresholdStorms(self,data,threshold):
        ...
        while cur_row < num_rows:
            ...
            if data[RequiredHeaders.WAVE_SIG_HEIGHT][cur_row] >= threshold:
                ...
                while (data[RequiredHeaders.WAVE_SIG_HEIGHT][cur_row] >= threshold) or (downtime_seconds <= self.MAX_STORM_DOWNTIME):
                    ...
                    if data[RequiredHeaders.WAVE_SIG_HEIGHT][cur_row] >= threshold:
                        duration_seconds = int(data[RequiredHeaders.TIME_UNIX][cur_row]) - int(data[RequiredHeaders.TIME_UNIX][start_row])
                        downtime_seconds = 0
                    else:
                        downtime_seconds = int(data[RequiredHeaders.TIME_UNIX][cur_row]) - int(data[RequiredHeaders.TIME_UNIX][start_row])
                ...
                if duration_seconds >= self.MIN_STORM_DURATION and downtime_seconds >= self.MAX_STORM_DOWNTIME:
    ...
    def runPOT(self,data_frame):
        ...
        storms_list = self.peakOverThresholdStorms(data_frame,self.calcThreshold(data_frame))
