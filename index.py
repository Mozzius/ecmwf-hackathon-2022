...
def extract_data(file_path, lat, lon):
    ...
    print(
        "Closest location in data to {0}N{1}E = {2}N{3}E".format(
            lat, lon, lat_arr[lat_ind], lon_arr[lon_ind]
        )
    )

    print(
        "Num locations = {0} * {1} = {2}".format(
            len(lat_arr), len(lon_arr), len(lat_arr) * len(lon_arr)
        )
    )
    ...
    for var in MET_VARIABLES:
        ...
        return_df[var] = f.variables[var][
            0 : int(len(time_arr)), (lat_ind) : (lat_ind + 1), (lon_ind) : (lon_ind + 1)
        ].flatten()
...
def convertU10V10toSpdDir(pandas_df):
    ...
    for j in range(pandas_df.shape[0]):
        tmp_wind_speed[j] = math.sqrt(
            math.pow(pandas_df.at[j, "u10"], 2) * math.pow(pandas_df.at[j, "v10"], 2)
        )
        tmp_wind_dir[j] = math.fmod(
            360.0 + math.atan2(pandas_df.at[j, "u10"], pandas_df.at[j, "v10"]), 360
        )
...
if __name__ == "__main__":
    ...
    data_frame.to_csv(
        "extracted_wind_with_spd_dir_monthly_{0}N{1}E.csv".format(LAT, LON)
    )
    ...
    plots.plotExportHorizontalBarCode(
        data_frame["wind_dir_rad"].to_numpy(),
